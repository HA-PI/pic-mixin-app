...
;(function(win,$){
    ...
    sS.prototype.init = function(options){
        ...
        if(me.opts.continuousScroll) {
            me.opts.ul.prepend(me.opts.li.last().clone()).append(me.opts.li.first().clone());
        }
    ...
    // css过渡
    function fnTransition(me, dom, num,index){
        dom.css({
            '-webkit-transition':'all '+num+'s '+me.opts.transitionType,
            'transition':'all '+num+'s '+me.opts.transitionType
        });
        // if(isNaN(index)) return;
        // dom.find('li').eq(index+1).css('visibility','visible');
    }

    // css位移
    function fnTranslate(me, dom, distance,index){
        distance=distance>0?Math.ceil(distance):Math.floor(distance);
        var result = me.opts.axisX ? distance+'px,0' : '0,'+distance+'px,0';
        dom.css({
        ...
        // if(isNaN(index)) return;
        // setTimeout(function(){
        //     dom.find('li').eq(index+1).siblings().css('visibility','hidden');
        // },300);
    }
    ...
    function fnScroll(me, num){
        fnTransition(me, me.opts.ul, num,me._index);
        fnTranslate(me, me.opts.ul, -me._index*me._slideDistance,me._index);
    }
